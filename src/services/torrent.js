...
const humanize = require("humanize-data");
const UserModel = require("../models/User");
const CustomError = require("../utils/error");
const lowdb = require("../config/lowdb");
const { result } = require("../config/lowdb");

/**
 * Parse a torrent from magnet and returns its info.
 * @param {*} magnet Magnet URL of torrent file, e.g magnet://?...
 */
async function getTorrentInfo(magnet) {
  try {
    const torrent = parser(magnet);
...
async function addTorrent(user, torrent) {
  /**
  ...
  if (!!existingTorrent) {
    ...
    if (!isDuplicate) {
      return reject(
        CustomError("Torrent already exists.", 400, "DuplicateTorrent")
      );
    }
    ...
    return {
      torrent: existingTorrent.toObject(),
      alreadyExists: true,
    };
  }
  ...
  return { torrent: torrent, alreadyExists: false };
}
